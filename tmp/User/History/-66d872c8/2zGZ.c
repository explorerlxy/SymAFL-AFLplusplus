  ...
  for (iter = 0; iter < afl->foreign_sync_cnt; iter++) {
    ...
    if (afl->foreign_syncs[iter].dir && afl->foreign_syncs[iter].dir[0]) {
      ...
      for (i = 0; i < (u32)nl_cnt; ++i) {
        ...
        if(afl->symcc_mode){
          int32_t depth = path_con_tree_check_input(afl->path_con_tree, mem, st.st_size);
          if(depth == -1){
...
#undef MSG_FORK_ON_APPLE
    ...
    if(afl->symcc_mode && unlikely(!q->cal_failed) && unlikely(!q->var_behavior)){
      int32_t depth = path_con_tree_check_input(afl->path_con_tree, use_mem, read_len);
      if(depth >= 0){
        memcpy(afl->outdir->map, afl->out_dir, strlen(afl->out_dir));
        ...
        *(u32*)afl->insert_depth->map = depth;
        ...
        path_con_tree_insert_trace(afl->path_con_tree, path_con_trace_path, q);
  ...
  if(afl->symcc_mode){
    u8 *path_con_tree_path;
    path_con_tree_path = alloc_printf("%s/queue/PathConTree-Init", afl->out_dir);
    visualize_path_con_tree(afl->path_con_tree, "PathConTree-Init");
    ck_free(path_con_tree_path);
  }
...
void setup_outdir_shmem(afl_state_t *afl) {
  ...
  if (!map) { FATAL("BUG: Zero return from afl_shm_init."); }
  memcpy(map, afl->out_dir, strlen(afl->out_dir));
